...
5. Create the GitHub repo, then run `bash scripts/bootstrap-github.sh` from inside it (needs `gh` authenticated) — sets up dependabot labels, squash-only merge policy, Actions permissions for release-please, and the `main` ruleset (PR required, `ci` as a required status check, no direct push). Re-run it any time: every step is idempotent.
